...
class QFormerLayer(nn.Module):
    ...
    def forward(self, queries, img_patches, token_embed, mask_tokens, text_attn_mask=None):
        text_pad_mask = None
...
class MaskedClassifier(nn.Module):
    def __init__(
        ...
        dropout     = 0.1,
        use_mask    = True,     # NEW
        pooling     = "dual",   # NEW: "dual" | "mean"
    ):
        super().__init__()

        self.use_mask = use_mask     # NEW
        self.pooling  = pooling      # NEW
    ...
    def forward(self, img_patches, cls_embed, token_embed, mask, attn_mask=None):
        mask_tokens = self.mask_encoder(mask) if self.use_mask else None   # CHANGED

        queries = self.qformer(img_patches, cls_embed, token_embed, mask_tokens, attn_mask)

        if self.pooling == "mean":                       # NEW branch
            pooled = queries.mean(dim=1)
        else:
            type_signal = self.type_pre(
                queries.mean(dim=1)
            ).detach()
            pooled = self.pool(queries, type_signal)
